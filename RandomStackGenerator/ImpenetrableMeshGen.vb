...
Public Class WaterGen
    ...
    ''' <param name="settMap">Общие настройки для карты</param>
    Public Sub Gen(ByRef m As Map, ByRef settMap As Map.SettingsMap, ByRef impMG As ImpenetrableMeshGen)
    ...
    Public Class WaterPlacer_Common
        ...
        Public Function MakeWaterSufaceTestArray(ByRef pos As Point, ByRef w As WaterPlacer_2.WaterBlock, _
                                                 Optional ByRef currentWaterSurface(,) As Boolean = Nothing, _
                                                 Optional ByRef wasGround(,) As Boolean = Nothing) As Boolean(,)
            Dim result(,) As Boolean
            If IsNothing(currentWaterSurface) Then
                ReDim result(m.xSize, m.ySize)
                For y As Integer = 0 To m.ySize Step 1
                    For x As Integer = 0 To m.xSize Step 1
                ...
                Next y
            Else
                result = currentWaterSurface
            End If
            For y As Integer = 0 To w.yMax Step 1
                For x As Integer = 0 To w.xMax Step 1
                    If w.GetValue(x, y) Then
                        If Not IsNothing(wasGround) And Not result(pos.X + x, pos.Y + y) Then wasGround(x, y) = True
                        If m.symmID > -1 Then
    ...
    Public Class WaterPlacer_2
        ...
        End Class
        Private Class CommonWaterPlacingData
            Public currentWaterSurface(Environment.ProcessorCount - 1)(,) As Boolean
            Public wasGround(Environment.ProcessorCount - 1)(,) As Boolean
            Public addsWaterAmount(,) As Integer
            Public isChanged(,) As Boolean
            Public selector As RandomSelection
            ...
            Public Sub New(ByRef w As WaterPlacer_2, ByRef pointsAmount As Integer, ByRef commonPlacing As Boolean)
                ...
                For k As Integer = 0 To UBound(myBlocksKeys) Step 1
                    For i As Integer = 0 To pointsAmount - 1 Step 1
                        ...
                        itemPos(n)(itemPosData.pointIndex) = i
                        itemID(k, i) = n
                        selector.Add(n)
                    Next i
                Next k
            ...
            Private Enum itemPosData
                wBlocksKeysIndex = 0
                pointIndex = 1
            End Enum

            Public Function GetWBlocksKeysIndex(ByRef selected As Integer) As Integer
                Return itemPos(selected)(itemPosData.wBlocksKeysIndex)
            End Function
            Public Function GetPointIndex(ByRef selected As Integer) As Integer
                Return itemPos(selected)(itemPosData.pointIndex)
            End Function
        End Class
        ...
        Private Sub AddWaterCommon(ByVal loc As Location, ByVal fpInfo As FreePointsInfo, ByRef WaterAmount As Integer)
            ...
            Do While WaterAmount > 0 And weightSum > 0
                ...
                If weightSum > 0 Then
                    ...
                    Call ResetChanged(cData.isChanged, key, x, y, False)
                End If
            Loop
        End Sub
        ...
        Private Function CanAddWatherAmount(ByRef loc As Location, ByRef fpInfo As FreePointsInfo, ByRef WaterAmount As Integer, _
            ...
            If Not IsNothing(cData.isChanged) Then
                ...
                For j As Integer = p.Y To y2 Step 1
                    ...
                    End If
                Next j
                If Not changed Then Return cData.addsWaterAmount(wBlockIndex, pointIndex)
            End If
            ...
            If addsWaterAmount > 0 Then
                Dim wTest(,) As Boolean = owner.wpCommon.MakeWaterSufaceTestArray _
                                          (p, w, cData.currentWaterSurface(proc), cData.wasGround(proc))
                Dim returnZero As Boolean = False
                For Each u As WaterBlock In unacceptWBlocks.Values
                    If TestByUnacceptableBlock(wTest, u, w, p) Then
                        returnZero = True
                        Exit For
                    End If
                Next u
                If Not IsNothing(cData) Then
                    For y As Integer = 0 To w.yMax Step 1
                        For x As Integer = 0 To w.xMax Step 1
                            If cData.wasGround(proc)(x, y) Then
                                cData.wasGround(proc)(x, y) = False
                                If m.symmID > -1 Then
                                    Dim pp() As Point = owner.symm.ApplySymm(New Point(p.X + x, p.Y + y), settMap.nRaces, m, 1)
                                    For k As Integer = 0 To UBound(pp) Step 1
                                        Dim tx As Integer = pp(k).X
                                        Dim ty As Integer = pp(k).Y
                                        cData.currentWaterSurface(proc)(tx, ty) = False
                                    Next k
                                Else
                                    cData.currentWaterSurface(proc)(p.X + x, p.Y + y) = False
                                End If
                            End If
                        Next x
                    Next y
                End If
                If returnZero Then addsWaterAmount = 0
            End If
        ...
        Private Sub AddWater(ByRef WaterAmount As Integer, ByRef key As String, ByRef x As Integer, ByRef y As Integer, ByVal useObjWBlocks As Boolean, Optional ByRef isWaterSurface()(,) As Boolean = Nothing)
            ...
            Next j
        End Sub
        Private Sub ResetChanged(ByRef isChanged(,) As Boolean, ByRef key As String, ByRef x As Integer, ByRef y As Integer, ByVal useObjWBlocks As Boolean)
            For j As Integer = 0 To m.ySize Step 1
                For i As Integer = 0 To m.xSize Step 1
                    isChanged(i, j) = False
                Next i
            ...
            Dim x2 As Integer = x + w.xMax
            Dim y2 As Integer = y + w.yMax
            Dim b As New Location.Borders
            For j As Integer = y To y2 Step 1
                If YFilter(j) Then
                    For i As Integer = x To x2 Step 1
                        If XFilter(i) Then
                            If w.GetValue(i - x, j - y) Then
                                b.maxX = Math.Min(x + wBlockUnacceptMaxSize.X + 1, m.xSize)
                                b.minX = Math.Max(x - wBlockUnacceptMaxSize.X - wBlockMaxSize.X - 2, 0)
                                b.maxY = Math.Min(y + wBlockUnacceptMaxSize.Y + 1, m.ySize)
                                b.minY = Math.Max(y - wBlockUnacceptMaxSize.Y - wBlockMaxSize.Y - 2, 0)
                                For by As Integer = b.minY To b.maxY Step 1
                                    For bx As Integer = b.minX To b.maxX Step 1
                                        If m.symmID > -1 Then
                                            Dim pp() As Point = owner.symm.ApplySymm(New Point(bx, by), settMap.nRaces, m, 1)
                                            For k As Integer = 0 To UBound(pp) Step 1
                                                isChanged(pp(k).X, pp(k).Y) = True
                                            Next k
                                        Else
                                            isChanged(bx, by) = True
                                        End If
                                    Next bx
                                Next by
                            End If
...
Public Class ObjectsContentSet
    ...
    Public Sub New(ByRef RStack As RandStack)
        ...
        ReDim itemCostFilter(UBound(randStack.AllItems))
        For i As Integer = 0 To UBound(randStack.AllItems) Step 1
            If Not randStack.comm.IsExcluded(randStack.AllItems(i)) Then
                Dim sum As Integer = AllDataStructues.Cost.Sum(randStack.LootCost(randStack.AllItems(i)))
                If sum > 0 Then
                    typeMinCost(randStack.AllItems(i).type) = Math.Min(typeMinCost(randStack.AllItems(i).type), sum)
                    typeMaxCost(randStack.AllItems(i).type) = Math.Max(typeMaxCost(randStack.AllItems(i).type), sum)
                End If
            End If
            If randStack.AllItems(i).itemCost.Gold = 0 Then
                itemCostFilter(i) = False
            ElseIf Not randStack.LootCost(randStack.AllItems(i)).Gold = randStack.AllItems(i).itemCostSum Then
                itemCostFilter(i) = False
            Else
                itemCostFilter(i) = True
            End If
        Next i
...
#Region "Mage merchant"
    ...
    Public Function MakeSpellsList(ByRef d As AllDataStructues.DesiredStats, ByVal AllManaSources As List(Of String), _
        ...
        Dim txt As String = ""
        Dim s() As String
        Dim mode As SpellsMerchantMod

        For Each v As String In d.shopContent
            s = v.Split(CChar("#"))
            mode = CType(s(0), SpellsMerchantMod)

            If log.IsEnabled Then txt = "In: " & v & " -> "
            If mode = SpellsMerchantMod.ID Then
                If Not res.Contains(s(1).ToUpper) Then
                    res.Add(s(1).ToUpper)
                    If log.IsEnabled Then txt &= s(1).ToUpper
                Else
                    txt = ""
                End If
            ElseIf mode = SpellsMerchantMod.LevelAndRaceAndMass Then
                Dim prop As String = s(1)
                Dim selected As Integer = SelectSpell(prop, -1, races, spellAvailResourcesFilter, res, selection)
                If selected > -1 Then res.Add(randStack.AllSpells(selected).spellID)
                If log.IsEnabled Then txt &= SpellMsgToLog(selected)
            ElseIf mode = SpellsMerchantMod.Type Then
                If Not selection.Count = 0 Then Throw New Exception("Unexpected selection.Count: " & selection.Count)
                Dim type As Integer = CInt(s(1))
                Parallel.For(0, Environment.ProcessorCount, _
                 Sub(proc As Integer)
                     Dim add As Boolean
                     For u As Integer = proc To UBound(randStack.AllSpells) Step Environment.ProcessorCount
                ...
                If selected > -1 Then res.Add(randStack.AllSpells(selected).spellID)
                If log.IsEnabled Then txt &= SpellMsgToLog(selected)
            ElseIf mode = SpellsMerchantMod.TypeAndLevelAndRaceAndMass Then
                Dim type As Integer = CInt(s(1))
                Dim prop As String = s(2)
                Dim selected As Integer = SelectSpell(prop, type, races, spellAvailResourcesFilter, res, selection)
                If selected > -1 Then res.Add(randStack.AllSpells(selected).spellID)
                If log.IsEnabled Then txt &= SpellMsgToLog(selected)
            Else
                Throw New Exception("Unexpected mode in spells merchant: " & mode)
            End If
        ...
        Return res
    End Function
    Private Function SpellMsgToLog(ByRef spellID As Integer) As String
        If spellID > -1 Then
            Dim txt As String = randStack.AllSpells(spellID).spellID & " - " & _
                                randStack.AllSpells(spellID).name & " - " & _
                                "Lvl: " & randStack.AllSpells(spellID).level & " - " & _
                                "Cat: " & randStack.AllSpells(spellID).category
            Return txt
        Else
            Return "nothing"
        End If
...
#Region "Units merchant"
    ...
    Public Function MakeMercenariesList(ByRef d As AllDataStructues.DesiredStats, _
        ...
        Dim txt As String = ""
        Dim s() As String
        Dim mode As UnitsMerchantMod

        For Each v As String In d.shopContent
            s = v.Split(CChar("#"))
            mode = CType(s(0), UnitsMerchantMod)

            If log.IsEnabled Then txt = "In: " & v & " -> "
            If mode = UnitsMerchantMod.ID Then
                If Not res.Contains(s(1).ToUpper) Then
                    res.Add(s(1))
                    If log.IsEnabled Then txt &= s(1).ToUpper
                Else
                    txt = ""
                End If
            ElseIf mode = UnitsMerchantMod.ExpNext Then
                Dim expNext As Integer = CInt(s(1))
                Dim selected As Integer = SelectMercenary(expNext, -1, -1, res, selection)
                If selected > -1 Then res.Add(randStack.AllUnits(selected).unitID)
                If log.IsEnabled Then txt &= UnitMsgToLog(selected)
            ElseIf mode = UnitsMerchantMod.ExpKilled Then
                Dim expKilled As Integer = CInt(s(1))
                Dim selected As Integer = SelectMercenary(-1, expKilled, -1, res, selection)
                If selected > -1 Then res.Add(randStack.AllUnits(selected).unitID)
                If log.IsEnabled Then txt &= UnitMsgToLog(selected)
            ElseIf mode = UnitsMerchantMod.Race Then
                If Not selection.Count = 0 Then Throw New Exception("Unexpected selection.Count: " & selection.Count)
                Dim race As Integer = randStack.comm.RaceIdentifierToSubrace(s(1))
                Parallel.For(0, Environment.ProcessorCount, _
                 Sub(proc As Integer)
                     Dim add As Boolean
                ...
                If selected > -1 Then res.Add(randStack.AllUnits(selected).unitID)
                If log.IsEnabled Then txt &= UnitMsgToLog(selected)
            ElseIf mode = UnitsMerchantMod.RaceAndExpNext Then
                Dim race As Integer
                If IsNumeric(s(1)) Then
                    race = CInt(s(1))
                Else
                    race = randStack.comm.RaceIdentifierToSubrace(s(1))
                End If
                Dim expNext As Integer = CInt(s(2))
                Dim selected As Integer = SelectMercenary(expNext, -1, race, res, selection)
                If selected > -1 Then res.Add(randStack.AllUnits(selected).unitID)
                If log.IsEnabled Then txt &= UnitMsgToLog(selected)
            ElseIf mode = UnitsMerchantMod.RaceAndExpKilled Then
                Dim race As Integer
                If IsNumeric(s(1)) Then
                    race = CInt(s(1))
                Else
                    race = randStack.comm.RaceIdentifierToSubrace(s(1))
                End If
                Dim expKilled As Integer = CInt(s(2))
                Dim selected As Integer = SelectMercenary(-1, expKilled, race, res, selection)
                If selected > -1 Then res.Add(randStack.AllUnits(selected).unitID)
                If log.IsEnabled Then txt &= UnitMsgToLog(selected)
            Else
                Throw New Exception("Unexpected mode in units merchant: " & mode)
            End If
            If Not txt = "" Then Call AddToLog(log, LogID, txt)
        Next v
        ...
        Return res
    End Function
    Private Function UnitMsgToLog(ByRef unitID As Integer) As String
        If unitID > -1 Then
        ...
        End If
    End Function
    Private Function SelectMercenary(ByVal expNext As Integer, ByVal expKilled As Integer, ByVal race As Integer, ByVal added As List(Of String), _
                                     ByVal selection As RandomSelection) As Integer

        Dim expNextTolerance As Integer
        Dim dExpNextTolerance As Integer = 50
        Dim expKilledTolerance As Integer
        Dim dExpKilledTolerance As Integer = 10
        Dim oneMore As Boolean = False

        If expNext < 0 Then expNextTolerance = 10000
        If expKilled < 0 Then expKilledTolerance = 10000

        If expNext < 0 And expKilled < 0 Then
            dExpNextTolerance = 10000
            dExpKilledTolerance = 10000
        ElseIf expNext < 0 Then
            dExpNextTolerance = 0
        ElseIf expKilled < 0 Then
            dExpKilledTolerance = 0
        End If

        If Not selection.Count = 0 Then Throw New Exception("Unexpected selection.Count: " & selection.Count)
        Do While oneMore Or (selection.Count = 0 And expNextTolerance <= 10000 And expKilledTolerance <= 10000)
            expNextTolerance += dExpNextTolerance
            expKilledTolerance += dExpKilledTolerance
            Parallel.For(0, Environment.ProcessorCount, _
             Sub(proc As Integer)
                 Dim add As Boolean
                 For u As Integer = proc To UBound(randStack.AllUnits) Step Environment.ProcessorCount
                     If randStack.comm.IsAppropriateFighter(randStack.AllUnits(u)) Then
                         If Not added.Contains(randStack.AllUnits(u).unitID) Then
                             add = True
                             If add And expNext > -1 Then
                                 If randStack.AllUnits(u).small Then
                                     add = (Math.Abs(randStack.AllUnits(u).EXPnext - expNext) <= expNextTolerance)
                                 Else
                                     add = (Math.Abs(randStack.AllUnits(u).EXPnext - 2 * expNext) <= 2 * expNextTolerance)
                                 End If
                             End If
                             If add And expKilled > -1 Then
                                 If randStack.AllUnits(u).small Then
                                     add = (Math.Abs(randStack.AllUnits(u).EXPkilled - expKilled) <= expKilledTolerance)
                                 Else
                                     add = (Math.Abs(randStack.AllUnits(u).EXPkilled - 2 * expKilled) <= 2 * expKilledTolerance)
                                 End If
                             End If
...
#Region "Items merchant"
    ...
    Public Function MakeMerchantItemsList(ByRef d As AllDataStructues.DesiredStats, _
        ...
        For Each v As String In d.shopContent
            s = v.Split(CChar("#"))
            mode = CType(s(0), ItemsMerchantMod)

            If log.IsEnabled Then txt = "In: " & v
            If mode = ItemsMerchantMod.ID Then
                res.Add(s(1).ToUpper)
                If log.IsEnabled Then txt &= " -> " & s(1).ToUpper
            ElseIf mode = ItemsMerchantMod.Cost Then
            ...
            ElseIf mode = ItemsMerchantMod.Type Then
                If Not selection.Count = 0 Then Throw New Exception("Unexpected selection.Count: " & selection.Count)
                Dim type As Integer = randStack.comm.itemTypeID.Item(s(1).ToUpper)
                Parallel.For(0, Environment.ProcessorCount, _
                 Sub(proc As Integer)
                     Dim add As Boolean
            ...
            ElseIf mode = ItemsMerchantMod.TypeAndCost Then
                Dim type As Integer
                If IsNumeric(s(1)) Then
                    type = CInt(s(1))
                Else
                    type = randStack.comm.itemTypeID.Item(s(1).ToUpper)
                End If
                Dim bar As Integer = CInt(s(2))
            ...
            Else
                Throw New Exception("Unexpected mode in items merchant: " & mode)
            End If
        ...
        Return res
    End Function
    Private Function ItemMsgToLog(ByRef itemID As Integer, ByRef dCost As Integer, ByRef addDeltaCost As Boolean) As String
        Dim txt As String = ""
...
#Region "Content settings creation"
    Private Function GetSettingsCommon(ByVal mode As Integer, ByRef input As List(Of String)) As List(Of String)
        Dim output As New List(Of String)
        For Each item In input
            output.Add(mode & "#" & item.ToUpper)
        Next item
        Return output
    End Function
...
#Region "Spells merchant"
    ...
    Public Function GetSpellsListSettings(ByVal mode As Integer, ByRef input As List(Of String)) As List(Of String)
        ...
        If mode = SpellsMerchantMod.RandomAtEachLine Then
            output = GetRandomMode(input, {0.25, 1, 1, 1}, AddressOf GetSpellsListSettings)
        ElseIf mode = SpellsMerchantMod.ID Then
            output = GetSettingsCommon(mode, input)
        Else
            For Each item As String In input
                ...
                If Not spell.spellID = "" Then
                    If randStack.comm.IsPreserved(spell) Then
                        output.Add(SpellsMerchantMod.ID & "#" & spell.spellID.ToUpper)
                    Else
                        Dim category As String = spell.category.ToString
                        Dim prop As String = SpellProperties(spell)
                        If mode = SpellsMerchantMod.LevelAndRaceAndMass Then
                            output.Add(mode & "#" & prop)
                        ElseIf mode = SpellsMerchantMod.Type Then
                            output.Add(mode & "#" & category)
                        ElseIf mode = SpellsMerchantMod.TypeAndLevelAndRaceAndMass Then
                            output.Add(mode & "#" & category & "#" & prop)
                        Else
...
#Region "Items merchant"
    ...
    Public Function GetMerchantListSettings(ByVal mode As Integer, ByRef input As List(Of String)) As List(Of String)
        ...
        If mode = ItemsMerchantMod.RandomAtEachLine Then
            output = GetRandomMode(input, {0.25, 1, 1, 1}, AddressOf GetMerchantListSettings)
        ElseIf mode = ItemsMerchantMod.ID Then
            output = GetSettingsCommon(mode, input)
        Else
            For Each item In input
                ...
                If Not thing.itemID = "" Then
                    If randStack.comm.IsPreserved(thing) Then
                        output.Add(ItemsMerchantMod.ID & "#" & item.ToUpper)
                    Else
                        ...
                        If mode = ItemsMerchantMod.Cost Then
                            output.Add(mode & "#" & cost)
                        ElseIf mode = ItemsMerchantMod.Type Then
                            output.Add(mode & "#" & type)
                        ElseIf mode = ItemsMerchantMod.TypeAndCost Then
                            output.Add(mode & "#" & type & "#" & cost)
                        Else
                            Throw New Exception("Unknown mode: " & mode)
                            output = Nothing
                        End If
...
#Region "Units merchant"
    ...
    ''' <param name="mode">-1 - случайный мод в каждой строчке, 1 - вернет ID, 2 - Планка опыта, 3 - Раса, 4 - Раса#Планка опыта, 5 - Опыт за убийство, 6 - Раса#Опыт за убийство</param>
    ...
    Public Function GetMercenariesListSettings(ByVal mode As Integer, ByRef input As List(Of String)) As List(Of String)
        ...
        If mode = UnitsMerchantMod.RandomAtEachLine Then
            output = GetRandomMode(input, {0.25, 1, 1, 1, 1, 1}, AddressOf GetMercenariesListSettings)
        ElseIf mode = UnitsMerchantMod.ID Then
            output = GetSettingsCommon(mode, input)
        Else
            For Each item In input
                ...
                If Not unit.unitID = "" Then
                    ...
                    Else
                        ...
                        ElseIf mode = UnitsMerchantMod.Race Then
                            output.Add(mode & "#" & race)
                        ElseIf mode = UnitsMerchantMod.RaceAndExpNext Then
                            output.Add(mode & "#" & race & "#" & expNext.ToString)
                        ElseIf mode = UnitsMerchantMod.ExpKilled Then
                            output.Add(mode & "#" & expKilled.ToString)
                        ElseIf mode = UnitsMerchantMod.RaceAndExpKilled Then
                            output.Add(mode & "#" & race & "#" & expKilled.ToString)
                        Else
                            Throw New Exception("Unknown mode: " & mode)
                            output = Nothing
                        End If
