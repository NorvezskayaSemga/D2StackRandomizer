...
#End Region
    ...
    Friend Function ItemCustomCost(ByRef item As AllDataStructues.Item) As AllDataStructues.Cost
        Dim customCost As String = ""
        If LootItemValue.ContainsKey(item.itemID.ToUpper) Then
            customCost = LootItemValue.Item(item.itemID.ToUpper)
        ElseIf LootItemValue.ContainsKey(itemType.Item(item.type).ToUpper()) Then
            customCost = LootItemValue.Item(itemType.Item(item.type).ToUpper())
        End If
        If Not customCost = "" Then
            If IsNumeric(customCost) Then
                Return item.itemCost * CDbl(customCost)
            Else
                Return AllDataStructues.Cost.Read(customCost)
            End If
        End If
